...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   if (!$api->validateCsrf($_POST['csrf_token'] ?? '')) {
      Redirect::withError('/views/contacts/', 'Invalid CSRF token. Please try again.');
   }

   // Confirm contact ID matches
   if (($_POST['contact_id'] ?? '') !== $contactId) {
      Redirect::withError('/views/contacts/', 'Invalid contact ID.');
   }

   // Process the operation
   $result = $api->deleteContact($contactId);

   // Handle the result
   if ($result['success']) {
      Redirect::withSuccess('/views/contacts/', $result['message']);
   } else {
      Redirect::withError('/views/contacts/', $result['message']);
   }
}
...
// Set page variables
$title = 'Delete Contact - ' . htmlspecialchars(($contactData['firstName'] ?? '') . ' ' . ($contactData['lastName'] ?? ''), ENT_QUOTES, 'UTF-8');
